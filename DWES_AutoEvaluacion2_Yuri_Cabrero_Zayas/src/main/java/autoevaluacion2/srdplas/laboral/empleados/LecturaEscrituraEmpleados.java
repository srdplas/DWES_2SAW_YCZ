...
public class LecturaEscrituraEmpleados {
	 ...
	 * @throws SQLException
	 * Lee fichero y escribe en otro indicado por parametros
	 */
	public void lecturaEmpleadosEscrituraNomina(String empleado, String nominas)
			throws IOException, NumberFormatException, DatosNoCorrectosException, SQLException {
		String nombreFichero = "../DWES_AutoEvaluacion2_Yuri_Cabrero_Zayas/src/main/java/autoevaluacion2/srdplas/laboral/empleados/"
				+ empleado;
		String nombreNominas = "../DWES_AutoEvaluacion2_Yuri_Cabrero_Zayas/src/main/java/autoevaluacion2/srdplas/laboral/empleados/"
				+ nominas;
		try {
			...
			while ((linea = br.readLine()) != null) {
				...
				if (empleados.length == 3) {
					...
					Empleado em = new Empleado(empleados[0], empleados[1], empleados[2].toCharArray()[0],
							Integer.parseInt(empleados[3]), Integer.parseInt(empleados[4]));
					bw.write(empleados[1] + ", " + nom.sueldo(em) + "\n");
					altaEmpleado(em);
				}
	 ...
	 * @throws DatosNoCorrectosException
	 * @throws SQLException
	 * Lee el fichero de empleados.txt, actualiza las nominas y escribe los empleados en empleados.backup
	 */
	public void copiaSeguridad(Empleado emp)
			throws NumberFormatException, IOException, DatosNoCorrectosException, SQLException {
		...
		try {
			...
			bw.write(emp.getNombre() + "," + emp.getDni() + "," + emp.getSexo() + "," + emp.getCategoria() + ","
					+ emp.getAnyosTrabajados() + '\n');
			bw.close();
	 ...
	 * 
	 * @param emp
	 * @throws IOException Escribe los empleados de la BBDD en el fichero
	 *                     empleados.txt
	 */
	 ...
	 * 
	 * @param con
	 * Da de alta e inserta un empleado en su tabla y actuliza tabla nominas
	 */
	public void altaEmpleado(Connection con) {
		...
		try {
			...
			if (vacio != false) {
				...
				st.execute("insert into empleados values('" + nombreEmpleado + "', '" + dniEmpleado + "', '"
						+ sexoEmpleado + "', " + categoriaEmpleado + ", " + anyosTrabajadosEmpleado + ");");

				// ACTUALIZAMOS EL SALARIO AUTOMATICAMENTE
				char stringToCharSexo = sexoEmpleado.charAt(0);
				Empleado trabajador = new Empleado(nombreEmpleado, dniEmpleado, stringToCharSexo, categoriaEmpleado,
						anyosTrabajadosEmpleado);
				...
				int numFilasAfectadas = st.executeUpdate("update nominas set sueldo=" + "'" + nuevoSalarioActualizado
	 ...
	 * @param emp
	 * @throws IOException
	 * Calcula la Nomina de un empleado y la escribe en el fichero  de nominas.dat
	 */
	 ...
	 * @param emp
	 * @throws SQLException
	 * @throws IOException  Da de alta a un empleado y lo inserta en la BBDD
	 */
	public void altaEmpleado(Empleado emp) throws SQLException, IOException { // ALTA EMPLEADO INTRODUCIENDOLE UN
																				// EMPLEADO
		...
		try {
			...
			st.execute("insert into Empleados values ('" + emp.getNombre() + "','" + emp.getDni() + "','"
					+ emp.getSexo() + "','" + emp.getCategoria() + "','" + emp.getAnyosTrabajados() + "')");
	...
	public void altaEmpleado(List<Empleado> empLista) throws SQLException, IOException { // ALTA EMPLEADO
																							// INTRODUCIENDOLE UN
																							// EMPLEADO
		Connection con = ConexionBBDD.getConnection();
		Statement st = con.createStatement();
	 ...
	 * 
	 * @param emp1
	 * @param emp2 Imprime datos y salario empleado que se le pasa por parametro
	 */
